...
Console.WriteLine("Дан первый отрезок: y = k1 * x + b1");
Console.WriteLine("Дан второй отрезок: y = k2 * x + b2");
...
{
    Console.WriteLine($"Точка пересечения отсутствует (прямые параллельны)");
}
